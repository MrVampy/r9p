...
const str = (value: string): [Uint8Array<ArrayBuffer>, bigint] => {
  const bytes = new Uint8Array(text.encode(value));
  return [bytes, BigInt(bytes.length)];
};

if (lib.symbols.r9p_front_abi_version() !== 17) {
...
console.log(
  `try: r9p -u demo -A / --bind 127.0.0.1:${port} cat /market/status`,
);
...
let tick = 0;
while (true) {
  const status = await lib.symbols.r9p_front_next_request(
    front,
    1000n,
    idOut,
    lenOut,
  ...
  if (status === 0) {
    ...
    const prefixLen = Number(
      lib.symbols.r9p_front_request_prefix_copy(
        front,
        requestId,
        new Uint8Array(0),
        0n,
    ...
    lib.symbols.r9p_front_request_prefix_copy(
      ...
      requestId,
      prefixBuf,
      BigInt(prefixBuf.length),
    );
    const buf = new Uint8Array(len);
    lib.symbols.r9p_front_request_copy(front, requestId, buf, BigInt(len));
    ...
    lib.symbols.r9p_front_complete_request(
      front,
      prefixBuf,
      BigInt(prefixBuf.length),
      requestId,
  ...
  lib.symbols.r9p_front_append_event(
    front,
    eventsPath,
    eventsPathLen,
    event,
