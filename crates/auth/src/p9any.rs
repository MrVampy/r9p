...
pub(crate) fn negotiate_client<S: Read + Write>(stream: &mut S, domain: &str) -> Result<()> {
    ...
    let expected = format!("{AUTH_PROTOCOL}@{domain}");
    if !fields.any(|field| field == expected) {
        return Err(Error::from(format!(
            "p9any server did not offer {expected}"
        )));
    }
    write_nul_string(stream, &format!("{AUTH_PROTOCOL} {domain}"))?;
...
pub(crate) fn negotiate_server<S: Read + Write>(stream: &mut S, domain: &str) -> Result<()> {
    write_nul_string(stream, &format!("{P9ANY_VERSION} {AUTH_PROTOCOL}@{domain}"))?;
    let selection = read_nul_string(stream, "p9any protocol selection")?;
    if selection != format!("{AUTH_PROTOCOL} {domain}") {
